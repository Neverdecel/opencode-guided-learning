...
this release. Therefore the final implementation uses the actual session path's
...
block per request (idempotently), so compaction does not remove the capability.
It does not maintain pending approvals outside the session; uncertain or lost
approval must be clarified, never inferred. Helper/subagent suppression is a
behavioral instruction: this hook's input contains no agent identifier.
